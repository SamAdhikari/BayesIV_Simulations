## Script to compute average treatment effects, heterogeneous average treatment effects,
##the length of credible interval and posterior coverage for simulation fits


##it uploads the saved .RData frame generated from 'Simulation_strongIV_n_100_example.R' script and saves the summary tables in a new .RData file
...
##list to temporarily store matrices
...
runModel_Normal_DPM = lapply(1:reps,
        function(x) runModel_Normal_DPM_list[[x]]
        )

runModels_Normal = lapply(1:reps,
            function(x) runModel_Normal_list[[x]]
            )
...
#########
### COMPUTE TREATMENT EFFECT ESTIMATES

#######     ################
##Average Treatment Effects

##FOR NON-PARAMETRIC BAYES IV MODEL
ATE_post_NDPM = lapply(1:reps,  function(ww){
    ATEpost_NDPM = getATE_posterior_NDPM(
                        fittedModel = runModel_Normal_DPM[[ww]],
                        X = X[,-1], niter = niter,
...
## FRO TWO STAGE LEAST SQUARES MODEL
...
CIwidthNDPM = sapply(1:reps, function(x) ATE_post_NDPM[[x]]$ATEci[3] - ATE_post_NDPM[[x]]$ATEci[1])

CIwidthNormal = sapply(1:reps, function(x) ATE_post_Normal[[x]]$ATEci[3] - ATE_post_Normal[[x]]$ATEci[1])

CIwidth2SLS = sapply(1:reps, function(x) ATEci_2SLS[[x]][3] - ATEci_2SLS[[x]][1])
...
#####
###HETEROGENOUS Average TREATMENT EFFECT##########
...
ATE_female_true =sapply(1:reps, function(x)
        mean(dataList[[x]]$Y1[femaleind] - dataList[[x]]$Y0[femaleind])
)

ATE_male_true = sapply(1:reps, function(x)
            mean(dataList[[x]]$Y1[maleind] - dataList[[x]]$Y0[maleind])
)

obs_Fem = mean( Yobs[which( D == 1 & X[, 3] == 0)]) - mean( Yobs[which( D == 0 & X[, 3] == 0)])

obs_Male = mean( Yobs[which( D == 1 & X[,3] == 1)]) - mean( Yobs[which( D == 0 & X[,3] == 1)])

## FOR TWO STAGE LEAST SQUARES MODEL
...
#width of CI
CIwidthNDPM_X = sapply(1:reps,function(x)
HATE_NDPM[[x]]$ATE_X_NDPM_ci[3] - HATE_NDPM[[x]]$ATE_X_NDPM_ci[1] )

CIwidthNormal_X = sapply(1:reps,function(x)
HATE_Normal[[x]]$ATE_X_Normal_ci[3] - HATE_Normal[[x]]$ATE_X_Normal_ci[1] )

CIwidth2SLS_X = sapply(1:reps,function(x)
HATEci_2SLS_M[[x]][3] - HATEci_2SLS_M[[x]][1] )
